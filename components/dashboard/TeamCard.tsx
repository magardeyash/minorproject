...
const ROLE_COLORS: Record<AssignedRole, string> = {
  CTO:        "bg-emerald-500/10 text-emerald-300 border-emerald-400/20",
  Developer:  "bg-blue-500/10 text-blue-300 border-blue-400/20",
  Designer:   "bg-indigo-500/10 text-indigo-300 border-indigo-400/20",
  Marketing:  "bg-pink-500/10 text-pink-300 border-pink-400/20",
  Sales:      "bg-cyan-500/10 text-cyan-300 border-cyan-400/20",
  Operations: "bg-teal-500/10 text-teal-300 border-teal-400/20",
}
...
export function TeamCard({
  ...
  return (
    <div className="glass-panel glass-panel-hover rounded-2xl p-5 border border-white/5 flex flex-col gap-4">
      {/* Avatar + Info */}
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="text-xs text-white/30 mt-0.5">For: <span className="text-accent-yellow font-semibold">{ideaTitle}</span></div>
        </div>
      </div>
