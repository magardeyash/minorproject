...
export function EmployeeLoginForm() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const parsed = LoginSchema.safeParse({ ...formData, role: "employee" })
    if (!parsed.success) {
  ...
  return (
    <AuthCard
      ...
      subtitle="Sign in to browse and apply to ideas"
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        >
          {isPending ? <Loader2 className="w-5 h-5 animate-spin" /> : "Sign In as Contributor"}
        </button>

        <p className="text-center text-accent-muted text-sm pt-2">
          Don&apos;t have an account?{" "}
          <Link href="/register/employee" className="text-btn hover:underline font-semibold">
